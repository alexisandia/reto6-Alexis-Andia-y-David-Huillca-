...
class Connection:
    def __init__(self, table_name):
        ...
        self.db = connect(host='localhost', user='postgres', password='Alexis---123',
                        database='semana_6', port=5432)
    ...
    def reference(self, nombre_atributo, nombre_tabla, id_reference): 
        query = f'''
                SELECT {nombre_atributo} FROM {nombre_tabla} WHERE id={id_reference}
            '''
        self.execute_query(query)
        return self.cursor.fetchall()